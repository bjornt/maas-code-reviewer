...
def _get_gemini_api_key(args: argparse.Namespace) -> str | None:
    ...
    Returns ``None`` if neither source provides a key.
    """
    if args.gemini_api_key_file:
        return Path(args.gemini_api_key_file).read_text().strip()
...
def _exit_missing_gemini_api_key() -> NoReturn:
    ...
    print(
        "Error: Gemini API key not provided. Use --gemini-api-key-file or set "
